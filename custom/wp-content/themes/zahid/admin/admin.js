...
jQuery(document).ready(function($){
	...
	jQuery('td').on('click','.remove-row',function(){
							  
	   jQuery(this).closest('tr').remove();										  
    });
	
	
	   jQuery('td').on('click','.add-row',function(){
	   ...
	   if(typeof tpl=='undefined' || tpl==''){
		...
		jQuery('td').on('click','.file-remove',function(){
	...
	jQuery('.file-wrapper').on('click','.file-browser',function(){
          ...
          window.send_to_editor = function(html) {
		    ...
		    $this.closest('div.file-wrapper').find('input.file').val(fileurl.src);	
		    $this.closest('div.file-wrapper').find('img.preview').attr('src',fileurl.src);
			tb_remove();
...
var link ='<tr>'; 
    link += '    <td class="right"><input size="60" type="text" name="setting[link_title][]" value="" /></td>';
	link += '    <td class="right"><input size="60" type="text" name="setting[link_detail][]" value="" /></td>';
	link += '    <td class="left"><a class="button remove-row">Remove</a></td>';
	link += '  </tr>';
/*End of row template*/